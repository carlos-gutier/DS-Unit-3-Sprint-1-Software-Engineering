...
Answer1: 
I appreciate with others write Docstrings to explain the function of their code.
Also well explained comments always come in handy, since each has a way to carry
out various tasks that may differ from the way one would. Proper indentation and
white space always make code readable. And last but not least, keeping with PEP8
style is appreciated.

Answer2:
Containers provide a minimal virtual operating system with all the sofware needed
to run applications. With containers code/programs can be run in a reliable and
reproducible manner. This makes testing programs a lot easier since these can be
tested and shared without having to adapt them to a local operating system. Using
containers would save Acme much time and headaches when testing and sharing code.
Containers are lightweight so these are specially helpful when the programs are large.
